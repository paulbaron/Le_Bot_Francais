...
NAME = f"DATA-LEN-{RATIOS_LEN}-SEQ-{SEQ_LEN}"
...
for RATIO in RATIOS:
    ...
    df = loadDataFrame(f'{DATAFOLDER}/{RATIO}.fed')
    
    df = df[1:]
    df = df[[f"close", f"high", f"low", f"volume"]]  # ignore the other columns besides price and volume
